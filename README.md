...
>
> **Pollinations key (recommended):** The anonymous tier is limited to square images, one queued request per IP, and a small daily quota. For reliable generation, get a free key at [enter.pollinations.ai](https://enter.pollinations.ai) and add it to `.env`:
>
> ```env
> POLLINATIONS_API_KEY=your_pollinations_key
> ```
>
> If the key is set, the agent sends it as `Authorization: Bearer` for higher limits and non-square sizes (e.g. `IMAGE_WIDTH=1200`, `IMAGE_HEIGHT=628`).
